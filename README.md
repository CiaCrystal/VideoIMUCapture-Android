...
For the simple region-classification experiment, first enable **Settings → Experiment → Enable 3×3 Target Grid** (off by default), then start ordinary foreground recording with the central round button. A 3×3 grid highlights one random cell. Tap the highlighted cell to complete the trial and show the next target. Cells are labelled `KEY_1` through `KEY_9` in reading order; `trial_id` starts at 1 in each recording and increments after every correct target tap. The active label and trial are written automatically into the existing `TOUCH_EVENT` TXT fields. Wrong/outside taps remain unlabelled with `trial_id=-1` and do not advance the experiment. Turning the setting off hides the grid and leaves touch labels empty with `trial_id=-1`.
