...
Stop a foreground recording with the central round button and wait for the `TXT saved` message. Stop background capture using **Stop and save** in the notification or **停止后台采集并保存** in the app, then wait for the saved notification. Background `video_recording.mp4` and `video_meta.txt` files are stored in the app's external files directory under a timestamped folder ending in `_background`. IMU sampling is requested at **100 Hz**. Pressing Home or locking the screen does not stop the background service; recording finishes when explicitly stopped or on a capture failure.
