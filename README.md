...
The two record controls have separate roles. The central round record button performs ordinary foreground recording, keeps the live camera preview visible, and writes `video_recording.mp4` plus `video_meta.txt` into a timestamped directory. **后台采集（视频 + IMU）** starts the background-capable recording service (version 0.13). Configure the camera first, then start background capture while the app is visible. Wait until the screen/notification reports that recording has started; you can then press Home, switch apps, or lock the screen. The service owns the camera, encoder, IMU and TXT writer with a partial wake lock, so live preview is intentionally unavailable only in this explicit background mode.

Stop a foreground recording with the central round button and wait for the `TXT saved` message. Stop background capture using **Stop and save** in the notification or **停止后台采集并保存** in the app, then wait for the saved notification. Background `video_recording.mp4` and `video_meta.txt` files are stored in the app's external files directory under a timestamped folder ending in `_background`. In **Settings → IMU Sampling**, accelerometer and gyroscope request rates can be selected independently from **50/100/200/300/400 Hz**; both default to 100 Hz and take effect when the next recording starts. The device and Android may deliver a lower actual rate. Pressing Home or locking the screen does not stop the background service; recording finishes when explicitly stopped or on a capture failure.

After the TXT writer has flushed and closed the file, the app also publishes a readable copy to `Download/VideoIMUCapture/<recording folder>/video_meta.txt`. Use this public copy with Android file managers or Windows USB/MTP; the original under `Android/data/.../files` is retained for app-internal pairing with the video but can be blocked by modern Android MTP implementations.
...
# Keyboard letter capture (A–Z)

On the capture screen, enable **键盘字母采集（A–Z）** (off by default), then start ordinary foreground recording with the round record button. The phone keyboard opens automatically; tap the keyboard prompt to reopen it after dismissing it. Choose an English keyboard and tap individual letters. The switch can be changed while recording and its preference is remembered. Keyboard mode temporarily hides the 3×3 experiment; turning it off restores the configured grid. Background capture does not collect keyboard input.

The app displays a fixed prompt and never displays or saves an accumulated text string. It saves individual uppercase letter annotations and timestamps in the existing UTF-8 `video_meta.txt` section/key=value format. `[KEYBOARD_CAPTURE]` records enable/disable transitions; `[KEYBOARD_EVENT]` records each accepted letter with `keyboard_label`, recording-local `keyboard_event_id`, `time_ns`, and `label_window_ns=200000000`. Repeated letters get separate IDs. When used, `IMU_DATA` and `OIS_SAMPLE` gain `keyboard_label`, `keyboard_event_id`, `keyboard_event_time_ns` and `keyboard_clock_comparable`; original fields (including grid `target_label`/`trial_id`) remain intact. Recordings that never enable keyboard capture retain their previous TXT schema.

Labels cover **[input callback timestamp, callback timestamp + 200 ms)**, ending earlier at the next letter or disable transition. Samples are matched using their own timestamps, including delayed OIS callbacks and interpolated IMU output. Outside a window, the label is empty and event ID/time are -1. Only cameras reporting `REALTIME` timestamps can be aligned to keyboard events; other cameras keep OIS data with `keyboard_clock_comparable=false` and empty keyboard labels. OIS samples themselves still require device/driver support and OIS reporting configuration.

Timing is `SystemClock.elapsedRealtimeNanos()` when the app receives input, **not the physical key DOWN/UP time**. Android IMEs normally deliver `commitText`/`setComposingText`, not raw touch events ([Android InputConnection](https://developer.android.com/reference/android/view/inputmethod/InputConnection)). Composition updates are deduplicated against their eventual commit. Multi-character additions are ignored because paste, swipe and suggestions do not provide individual key timing; numbers, punctuation and non-ASCII letters are ignored. Disable prediction/autocorrection and use direct English letter tapping for experiments. Different IMEs may batch or transform input, so this feature cannot guarantee physical keystroke timing or reconstruct a word delivered in one callback.

Device acceptance: record `a`, `b`, `b`, `Z`, pausing over 200 ms between taps; check four unique events and matching IMU/OIS IDs. Check rapid typing, composing/commit behavior, delete/retype, keyboard dismissal/reopening, toggle off/on during recording, stop while the keyboard is open, and a second recording with the feature off. Confirm no entered text appears on screen. These checks require the actual phone and IME; unit tests only verify decoding, timestamp windows, TXT association and session reset.
...
Record types are `RECORDING`, `EXPERIMENT_INFO`, `IMU_INFO`, `IMU_DATA`, `CAMERA_INFO`, `CAPTURE_CONFIG`, `FRAME_METADATA`, `OIS_SAMPLE`, `LENS_INTRINSICS_SAMPLE`, `TOUCH_EVENT` and (when unmatched) `FRAME_TIMESTAMP`. The output remains the same sectioned `video_meta.txt` file; protobuf is only the app's internal message representation. The `source` line identifies the sensor or subsystem. IMU vectors are ordered x, y, z: `gyroscope_rad_s`, `accelerometer_m_s2` and `magnetometer_uT`, with corresponding drift/bias and accuracy fields. `IMU_INFO` includes the device-reported sensor names/vendor information and resolutions, the independently requested accelerometer/gyroscope frequencies, and estimates measured from each sensor's callback timestamps. IMU samples use the gyroscope timestamp; acceleration and magnetic field values are interpolated onto that timestamp, not independent raw samples at that exact instant.

Camera records retain static capabilities, the requested capture configuration, intrinsics, distortion, stabilization, lens pose, resolution, exposure, ISO and detailed AF/lens state. Each frame always retains `camera_frame_number`; a matched frame also has `video_frame_number`. OIS and API 35+ lens-intrinsics samples are emitted as separate records linked by `parent_camera_frame_number`. Touch DOWN/UP events use an elapsed-realtime-compatible monotonic timestamp. Unmatched encoder timestamps are retained separately.

For the simple region-classification experiment, first enable **Settings → Experiment → Enable 3×3 Target Grid** (off by default), then choose **Target Selection Mode** directly below it. **Random** uses all nine targets as before. **Custom** reveals **Custom Target Numbers**, where one or more numbers from 1–9 can be selected; only those targets will appear. Start ordinary foreground recording with the central round button. Tap the highlighted cell to complete the trial and show the next target. Cells are labelled `KEY_1` through `KEY_9` in reading order; `trial_id` starts at 1 in each recording and increments after every correct target tap. The active label and trial are written automatically into the `TOUCH_EVENT` fields and, when OIS data is available, every `OIS_SAMPLE`. Wrong/outside touch events remain unlabelled with `trial_id=-1` and do not advance the experiment. With no active foreground grid trial, OIS samples also use an empty label and `trial_id=-1`. Turning the setting off hides the grid.

Normal autofocus capture defaults to Camera2 `CONTINUOUS_PICTURE` with `AF_TRIGGER=IDLE`. Existing installs are migrated from the former `TOUCH_AUTO/AUTO` default when mode 4 is available. Camera startup sends one AF `CANCEL` request before the continuous repeating request, and continuous AF does not set `LENS_FOCUS_DISTANCE`.
...
**Enable OIS defaults to OFF after installation but remains user-configurable when the camera advertises OIS ON support.** Cameras advertising both OFF and ON use the normal Camera2 toggle. For vendor implementations that anomalously advertise only `[1]` (ON), the switch is still enabled and the app can request OFF, but the driver may reject or ignore that unadvertised mode; `FRAME_METADATA.actual_optical_stabilization_mode` remains authoritative. The switch stays grey only when the selected camera does not expose OIS ON through Camera2 or the request key is unavailable. OIS sample reporting is a separate capability.
