...
**Enable OIS is forced off by the app.** The persisted switch value is reset to OFF and the capture request uses `LENS_OPTICAL_STABILIZATION_MODE_OFF`, including on vendor implementations that anomalously advertise only `[1]` (ON). The switch summary calls out this case; `FRAME_METADATA.actual_optical_stabilization_mode` remains authoritative because a camera driver may reject or ignore an unadvertised OFF request. OIS sample reporting is a separate capability.
